...
export class Header2Component implements OnInit {
  ...
  // patterns
  // userNamePattern = '/^(?![_.])(?!.*[_.]{2})[a-z@A-Z0-9._]+(?<![_.])$/';
  // passwordPattern = '/^(?![_.])(?!.*[_.]{2})[a-z@A-Z0-9._]+(?<![_.])$/';
  // userEmailPattern = '/^[a-zA-Z0-9._-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,100}[a-zA-Z0-9])?)*$/'
  ...
  createForms() {
    ...
    this.signInForm.controls.loginPassword.valueChanges.subscribe((response) => {
  ...
  changeUserName(event) {
    ...
    if (input) {
      ...
      if (!string) {
        ...
        this.signInForm.controls.userName.setValidators([Validators.required, Validators.minLength(10), Validators.maxLength(10)]);
        this.signInForm.controls.userName.updateValueAndValidity();
      } else {
        console.log('set email & username validators')
        this.signInForm.controls.userName.updateValueAndValidity();
        this.signInForm.controls.userName.setValidators([Validators.required, Validators.minLength(10), Validators.maxLength(50)]);
        this.signInForm.controls.userName.updateValueAndValidity();
      }
      if (string) {
        // if(input.indexOf("@") > 0) {
  ...
  //     "type": 2
  //   }
  //   this.http.post(`${this.apiUrl}api/signup`, data).subscribe((data: any) => {
  //     console.log(data)
  //     if (data.success) {
  //       if (data.success == true && this.authService.setLoginStatus(true, data.token)) {
  //         let returnData = false;
  ...
  onSubmitRegister() {
    ...
    this.http.post(`${this.apiUrl}api/signup`, data).subscribe((data: any) => {
      ...
      (error) => {
        ...
        console.log(error.error);
        console.log(error.error.msg);
        this.registerErrorMessage = error.error.msg;
        this.ngxLoader.hide();
      }
    )
  }
  ...
  get getUserImage() {
    return this.currentUserData && this.currentUserData.avatar ? this.apiUrl + 'images/auth/' + this.currentUserData.avatar : '../../../../assets/img/defaults/defaultAvatar.png';
    // if(this.currentUserData) {
    // return this.apiUrl + 'images/auth/' + this.currentUserData.avatar;
    // }
  }

  faceBookAuth() {
    this.AuthLogin(new firebase.default.auth.FacebookAuthProvider(), 'facebook')
  }
  googleAuth() {
  ...
  async AuthLogin(provider: any, providerName: string) {
    try {
      ...
      // console.log(error);
      window.alert('There is some issue from your provider, please use different authorisation.');
    }
  }

  onSearchSubmit() {
    if (this.searchForm.valid) {
      this.router.navigate([`/query/${this.searchForm.controls.keyword.value}/0/10`]);
    }
  }
